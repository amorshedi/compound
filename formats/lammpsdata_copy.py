...
#
# from mbuild2 import Box
# from mbuild2.utils.conversion import RB_to_OPLS
# from mbuild2.utils.sorting import natural_sort
# from scipy.constants import epsilon_0
from myimports import *

__all__ = ['write_lammpsdata']

def write_lammpsdata(structure, filename,latmat, atom_style='full',
                    unit_style='real',
                    detect_forcefield_style=True, nbfix_in_data_file=True,
                    use_urey_bradleys=False,
                    use_rb_torsions=True, use_dihedrals=True):
    ...
    unique_bond_types = dict(enumerate(structure.bond_types))
    if angles:
    ...
    with open(filename, 'w') as data:
        ...
        if atom_style in ['full', 'molecular']:
            if bonds:
                data.write('{:d} bond types\n'.format(len(set(structure.bond_types))))
            if angles:
        ...
        data.write('\n')
        lo = np.min(structure.coordinates,axis=0)
        hi = np.diag(latmat)
        for i, dim in enumerate(['x', 'y', 'z']):
            data.write('{0:.6f} {1:.6f} {2}lo {2}hi\n'.format(lo[i],lo[i]+hi[i], dim))
        # Box data
        if not np.allclose(latmat-np.diag(np.diagonal(latmat)),np.zeros([3,3])):
        ...
        if forcefield:
            ...
            if bonds:
                ...
                for idx,params in enumerate(structure.bond_types,1):
                    data.write('{}\t{}\t{}\n'.format(idx,params.k,params.req))
        ...
        if atom_style in ['full', 'molecular']:
            ...
            if bonds:
                data.write('\nBonds\n\n')
                cnt = 1;
                for i,bond in enumerate(structure.bonds,1):
                    if bond.type:
                        data.write('{:d}\t{:d}\t{:d}\t{:d}\n'.format(
                            cnt,structure.bond_types.index(bond.type)+1,
